
export const ROUTES = {
    HOME: "/",
    LOGIN: "/login",

    CONFIG_ROTA: "/admin/config/rotas",
    CONFIG_ROTA_NEW: "/admin/config/rotas/new",
    CONFIG_ROTA_EDIT: "/admin/config/rotas/",
    CONFIG_ROTA_VIEW: "/admin/config/rotas/view/",

    CONFIG_ACCOUNT: "/admin/config/accounts",
    CONFIG_ACCOUNT_NEW: "/admin/config/accounts/new",
    CONFIG_ACCOUNT_EDIT: "/admin/config/accounts/",
    CONFIG_ACCOUNT_VIEW: "/admin/config/accounts/view/",

    CONFIG_ACCOUNT_OPERATION: "/admin/config/accounts-operation",
    CONFIG_ACCOUNT_OPERATION_NEW: "/admin/config/accounts-operation/new",
    CONFIG_ACCOUNT_OPERATION_EDIT: "/admin/config/accounts-operation/",
    CONFIG_ACCOUNT_OPERATION_VIEW: "/admin/config/accounts-operation/view/",

    CONFIG_CONTRACT_MODEL: {
        ROOT: "/admin/config/contract-model",
        NEW: "/admin/config/contract-model/new",
        EDIT: "/admin/config/contract-model/",
        VIEW: "/admin/config/contract-model/view/",
    },

    CONFIG_USER: "/admin/config/users",
    CONFIG_USER_NEW: "/admin/config/users/new",
    CONFIG_USER_EDIT: "/admin/config/users/",
    CONFIG_USER_VIEW: "/admin/config/users/view/",
    CONFIG_USER_ROUTES: "/admin/config/users/user-rotas/",
}